# Здание 30.3.1
# п.2. Написать тест, который проверяет, что на странице со списком
# питомцев пользователя хотя бы у половины питомцев есть фото.
...
def test_photo_availability(go_to_my_pets):
    '''Поверка того, что на странице со списком питомцев
    пользователя хотя бы у половины питомцев есть фото'''
    ...
    # Проверка того, что количество питомцев с фотографией
    # больше или равно половине количества питомцев
